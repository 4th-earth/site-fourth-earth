...
class ContentBuilder extends Fold
{
    ...
    public function authenticatedHomeView()
    {
        $message = "";
        if (request()->session()->has("message")) {
            $message = session("message");
        ...
        $view = UIKit::webView(
            $this->pageTitle(),
            $message,
            UIKit::main()->attr("is transcript"),
            UIKit::aside()->attr("is status"),
            UIKit::aside(
                UIKit::nav(
                    UIKit::ul(
                        UIKit::li(
                            UIKit::anchor("Choose a character", "/")
                        )
                    ),
                    UIKit::h2("Creator capabilities"),
                    UIKit::ul(
                        UIKit::li(
                            UIKit::anchor("Update lore", "/lore")
                        )
                    ),
                    UIKit::h2("Settings & Security")
                )
            )->attr("is appearance"),
            UIKit::footer(
                UIKit::p("Copyright © Joshua Bruce, 2020. All rights reserved.")
